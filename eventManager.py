...
def eventManager(message: dict, msgSig: str):
    ...
    if class_name == "accounts":
        ...
        if (int(message['content']['autoBillCycle']) > 0 and
                (int(message['content']['retryNumber']) == 0 or
                    int(message['content']['retryNumber']) >= config.RETRY_COUNT
                )
            ):
            # ...and autobillCycle > 1 and retryNumber = 0 (first recurring
            #  billing decline) AutoBill billingState changed to 'In Retry'
            # (Soft Fail) or 'Unusable Payment Method' (Hard Fail)
            #
